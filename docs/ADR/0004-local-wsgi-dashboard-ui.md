...
Pages call existing application services; no separate REST backend.

## Consequences

- Zero new UI framework dependencies for M2
- Works on Windows/Linux/macOS with a browser
- No auth model — bind host must stay loopback on untrusted networks
- A native shell can wrap the same UI later without rewriting use cases
