...
2. **REST** — Wire with `create_api_context()` + `create_fastapi_app(ctx)`; `api/main.py` exposes `app = create_fastapi_app(...)` for uvicorn. Routes reuse `profile_intelligence.api.routes` handlers (shared with the legacy WSGI `ApiApp`).
