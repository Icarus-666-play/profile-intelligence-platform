...
2. **REST** — Wire with `create_application_context()` (bootstrap) + `create_fastapi_app(ctx)`; `api/main.py` exposes `app` for uvicorn. Routes reuse `profile_intelligence.api.routes` handlers (shared with the legacy WSGI `ApiApp`).
